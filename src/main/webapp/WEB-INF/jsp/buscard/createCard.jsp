<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<%
	String path = request.getContextPath()+"/";
	pageContext.setAttribute("contextPath", path);
%>


<!DOCTYPE html>
<html lang="zh-CN">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1,maximum-scale=1.0,user-scalable=0">
    <!-- 上述3个meta标签*必须*放在最前面，任何其他内容都*必须*跟随其后！ -->
    <title>注册商家</title>

    <!-- Bootstrap -->
    <link href="${contextPath}bootstrap/css/normalize.css" rel="stylesheet">
    <link href="${contextPath}bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link href="${contextPath}bootstrap/css/font-awesome.min.css" rel="stylesheet">
    <link href="${contextPath}bootstrap/css/font.css" rel="stylesheet">
    <link href="${contextPath}bootstrap/css/style.css" rel="stylesheet">
    <link href="${contextPath}bootstrap/css/plugin.css" rel="stylesheet">

    <!--[if lt IE 9]>
      <script src="//cdn.bootcss.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="//cdn.bootcss.com/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->
	
<style>
.content-main{
}
</style>
</head>
<body style="">
	<div class="content-main" style="font-size:1.4em;">
          <section class="panel">
            <div class="panel-body">
              <form class="form-horizontal" method="get" data-validate="parsley" action="${contextPath}buscard/createCardNext.do">
                <div class="form-group">
                  <label class="col-lg-3 control-label">卡名</label>
                  <div class="col-lg-8">
                    <input type="text" name="username" placeholder="商户号" data-required="true" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-lg-3 control-label">卡金额</label>
                  <div class="col-lg-8">
                    <input type="text" name="username" placeholder="店名" class="form-control">
                  </div>
                </div>
				
				<section class="panel">
					<div class="panel-body">
						
			<ul class="list-group">
			
			  <li class="list-group-item">
				<div class="form-group">
				  <div class="radio col-xs-6 control-label">
					<label class="radio-custom">
					  <input type="radio" name="radio">
					  <i class="fa fa-circle-o"></i>
						<strong>享受折扣</strong>
					</label>
				  </div>
				  <div class="col-xs-4">
					<input type="text" name="username" placeholder="9" class="form-control">
				  </div>
				  <div class="col-xs-2">折</div>
				</div>
              </li>
			  <li class="list-group-item">
				<div class="form-group">
				  <div class="radio col-xs-6 control-label">
					<label class="radio-custom">
					  <input type="radio" name="radio">
					  <i class="fa fa-circle-o"></i>
				  <strong>赠送金额</strong>
					</label>
				  </div>
				  <div class="col-xs-4">
					<input type="text" name="username" placeholder="0" class="form-control">
				  </div>
				  <div class="col-xs-2">元</div>
				</div>
              </li>
			  <li class="list-group-item">
				<div class="form-group">
				  <div class="radio col-xs-6 control-label">
					<label class="radio-custom">
					  <input type="radio" name="radio">
					  <i class="fa fa-circle-o"></i>
				  <strong>赠送项目</strong>
					</label>
				  </div>
				  <div class="col-xs-5">
					<input type="text" name="username" placeholder="店名" class="form-control">
				  </div>
				</div>
              </li>
			</ul>	
						
					</div>
				</section>
                <div class="form-group">
                  <label class="col-lg-3 control-label">使用范围</label>
                  <div class="col-lg-8">
                    <input type="text" name="username" placeholder="手机号" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-lg-3 control-label">有效期</label>
                  <div class="col-lg-8">
                    <input type="text" name="username" placeholder="手机号" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <div class="col-lg-9 col-lg-offset-3 text-center">                      
                    <button type="submit" class="btn btn-primary"> 下一步 </button>
                  </div>
                </div>
              </form>
            </div>
          </section>

</div>
	
	
	
  <script src="bootstrap/js/app/jquery.min.js"></script>
  <script src="bootstrap/js/bootstrap.min.js"></script>
	
  <script src="bootstrap/js/app/app.js"></script>
  <script src="bootstrap/js/app/app.plugin.js"></script>
  <script src="bootstrap/js/app/app.data.js"></script>
  <script src="bootstrap/js/app/fuelux/fuelux.js"></script>
  <script src="bootstrap/js/app/datepicker/bootstrap-datepicker.js"></script>
  <script src="bootstrap/js/app/slider/bootstrap-slider.js"></script>
  <script src="bootstrap/js/app/file-input/bootstrap.file-input.js"></script>
  <script src="bootstrap/js/app/combodate/moment.min.js"></script>
  <script src="bootstrap/js/app/combodate/combodate.js"></script>
  <script src="bootstrap/js/app/parsley/parsley.min.js"></script>
  <script src="bootstrap/js/app/select2/select2.min.js"></script>
  </body>
</html>